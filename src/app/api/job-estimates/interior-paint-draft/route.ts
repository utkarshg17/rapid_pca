...
type InteriorPaintDraftRequestBody = {
  estimate: {
    id: number;
    projectName: string;
    projectType: string;
...
export async function POST(request: Request) {
  ...
  let body: InteriorPaintDraftRequestBody;

  try {
    body = (await request.json()) as InteriorPaintDraftRequestBody;
  } catch {
    return NextResponse.json(
  ...
  if (!Array.isArray(body.areaTakeoffs) || body.areaTakeoffs.length === 0) {
    return NextResponse.json(
      { error: "Area takeoff rows are required for interior paint estimation." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    body: JSON.stringify({
      ...
      response_format: {
        type: "json_schema",
        json_schema: {
          name: "interior_paint_cost_draft",
          strict: true,
          schema: {
            type: "object",
    ...
    console.error(
      "Failed to read the OpenAI response for interior paint draft:",
      error
    );
    return NextResponse.json(
      {
        error:
          "OpenAI returned an unreadable response while generating interior paint draft.",
      },
      { status: 502 }
    );
  }
    ...
    return NextResponse.json(
      {
        error:
          ...
          "OpenAI request failed while generating interior paint draft.",
      },
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to parse the AI interior paint draft response." },
      { status: 500 }
    );
  }
}
