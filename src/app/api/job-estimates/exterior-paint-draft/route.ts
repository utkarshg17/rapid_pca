...
type ExteriorPaintDraftRequestBody = {
  estimate: {
    id: number;
    projectName: string;
    projectType: string;
...
export async function POST(request: Request) {
  ...
  let body: ExteriorPaintDraftRequestBody;

  try {
    body = (await request.json()) as ExteriorPaintDraftRequestBody;
  } catch {
    return NextResponse.json(
  ...
  if (!Array.isArray(body.areaTakeoffs) || body.areaTakeoffs.length === 0) {
    return NextResponse.json(
      { error: "Area takeoff rows are required for exterior paint estimation." },
      { status: 400 }
    );
  }
  ...
  const dsrReferenceRates = await fetchRelevantDsrRates({
    searchTerms: ["exterior paint", "paint", "primer", "putty", "weather coat", "texture"],
  ...
  const promptPayload = {
    ...
    costingContext: {
      ...
      finishDescription:
        ...
        "No user-entered exterior paint description provided. Assume a standard exterior paint system suitable for the project location.",
      notes: [
        "Estimate the total exterior paint area in sq.ft from the project details and room program.",
        "Do not jump directly to total quantity. First estimate the most appropriate benchmark area per GFA ratio for exterior paint based on project type, floor range, finish description, and building configuration, then multiply that benchmark by GFA to get the final total area.",
        `Use GFA = ${grossFloorAreaSqft.toFixed(2)} sq.ft, calculated as superstructure footprint x (stilt floor count + floor count).`,
        "If the exterior paint description is missing, assume a standard exterior paint system suited to the project location.",
        "Material cost per sq.ft should include primer, putty where applicable, paint coats, and miscellaneous consumables needed for the exterior paint system.",
        "Labour cost per sq.ft should include surface prep, putty work where applicable, primer application, paint application, and related finishing labour.",
        "Equipment cost per sq.ft may include scaffolding support, spray or roller equipment, and other clearly applicable paint equipment.",
  ...
  try {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an experienced Indian construction estimator preparing a conceptual exterior paint cost draft for estimator review. Use the project details, area takeoffs, and exterior paint finish description when available. Do not estimate the total area directly. First infer the most appropriate exterior paint area per GFA benchmark for this type of project and floor range, then multiply that benchmark by GFA to get the final total area. Estimate a realistic total exterior paint area in sq.ft, practical per-sq.ft material, labour, and equipment costs in INR, and keep assumptions short and specific. Use the provided dsrReferenceRates as the primary pricing anchor and return only valid JSON matching the schema.",
        },
        {
          role: "user",
      ...
      response_format: {
        type: "json_schema",
        json_schema: {
          name: "exterior_paint_cost_draft",
          strict: true,
          schema: {
            type: "object",
    ...
    console.error(
      "Failed to read the OpenAI response for exterior paint draft:",
      error
    );
    return NextResponse.json(
      {
        error:
          "OpenAI returned an unreadable response while generating exterior paint draft.",
      },
      { status: 502 }
    );
  }
    ...
    return NextResponse.json(
      {
        error:
          ...
          "OpenAI request failed while generating exterior paint draft.",
      },
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to parse the AI exterior paint draft response." },
      { status: 500 }
    );
  }
}
