...
export async function POST(request: Request) {
  ...
  const dsrReferenceRates = await fetchRelevantDsrRates({
    searchTerms: ["slab", "beam", "rcc", "concrete", "reinforcement", "formwork", "structural steel", "steel fabrication"],
  ...
  try {
      ...
      messages: [
        {
          ...
          content: isSteelStructure
            ? "You are an experienced Indian construction estimator preparing a conceptual cost draft for Upper Floors Construction (Slab + Beam) in a steel structure. Use the project details, area takeoffs, finishes, and benchmarkContext to estimate a realistic steel quantity in ton, and practical conceptual material, labour, and equipment costs in INR per ton. Do not estimate the total quantity directly. Start from benchmarkContext.finalRatioPerSqftGfa as the structural intensity anchor, convert it into a realistic ton-based benchmark for the selected steel system, and then multiply by GFA to get the final quantity. Material cost should primarily cover structural steel and necessary steel framing components. Labour cost should cover fabrication and erection labour. Use the provided dsrReferenceRates as the primary pricing anchor and return only valid JSON matching the schema."
            : "You are an experienced Indian construction estimator preparing a conceptual cost draft for Upper Floors Construction (Slab + Beam) in an RCC structure. Use the project details, area takeoffs, finishes, and benchmarkContext to estimate a realistic concrete quantity in cu.m, and practical conceptual material, labour, and equipment costs in INR per cu.m. Do not estimate the total quantity directly. Start from benchmarkContext.finalRatioPerSqftGfa as the default quantity anchor, adjust it only when the project inputs clearly justify it, and then multiply that benchmark by GFA to get the final quantity. Material cost must include concrete, steel, formwork, cover blocks, binding wire, and miscellaneous consumables. Labour cost must include rebar, formwork, and concrete pouring labour for slabs and beams. Use the provided dsrReferenceRates as the primary pricing anchor and return only valid JSON matching the schema.",
        },
        {
          role: "user",
    ...
    console.error(
      "Failed to read the OpenAI response for upper floors construction draft:",
      error
    );
    return NextResponse.json(
      {
        error:
          "OpenAI returned an unreadable response while generating upper floors construction draft.",
      },
      { status: 502 }
    );
  }
