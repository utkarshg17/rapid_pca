...
export async function POST(request: Request) {
  ...
  let body: VerticalStructuralElementsDraftRequestBody;

  try {
    body = (await request.json()) as VerticalStructuralElementsDraftRequestBody;
  } catch {
  ...
  if (!Array.isArray(body.areaTakeoffs) || body.areaTakeoffs.length === 0) {
    return NextResponse.json(
      {
        error:
          "Area takeoff rows are required for vertical structural elements estimation.",
      },
      { status: 400 }
    );
  }
    ...
    costingContext: {
      ...
      notes: isSteelStructure
        ...
        : [
            ...
            "Labour cost per cu.m must include rebar labour, formwork labour, concrete pouring labour, and related effort.",
  ...
  const openAiResponse = await fetch(openAiUrl, {
    ...
    body: JSON.stringify({
      ...
      response_format: {
        type: "json_schema",
        json_schema: {
          name: "vertical_structural_elements_cost_draft",
          strict: true,
          schema: {
            type: "object",
    ...
    return NextResponse.json(
      {
        error:
          ...
          "OpenAI request failed while generating vertical structural elements draft.",
      },
  ...
  try {
    ...
    return NextResponse.json(
      {
        error:
          "Failed to parse the AI vertical structural elements draft response.",
      },
      { status: 500 }
    );
  }
