...
type InteriorPlasterDraftRequestBody = {
  estimate: {
    id: number;
    projectName: string;
    projectType: string;
...
export async function POST(request: Request) {
  ...
  let body: InteriorPlasterDraftRequestBody;

  try {
    body = (await request.json()) as InteriorPlasterDraftRequestBody;
  } catch {
    return NextResponse.json(
  ...
  if (!Array.isArray(body.areaTakeoffs) || body.areaTakeoffs.length === 0) {
    return NextResponse.json(
      { error: "Area takeoff rows are required for interior plaster estimation." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    body: JSON.stringify({
      ...
      response_format: {
        type: "json_schema",
        json_schema: {
          name: "interior_plaster_cost_draft",
          strict: true,
          schema: {
            type: "object",
    ...
    console.error(
      "Failed to read the OpenAI response for interior plaster draft:",
      error
    );
    return NextResponse.json(
      {
        error:
          "OpenAI returned an unreadable response while generating interior plaster draft.",
      },
      { status: 502 }
    );
  }
    ...
    return NextResponse.json(
      {
        error:
          ...
          "OpenAI request failed while generating interior plaster draft.",
      },
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to parse the AI interior plaster draft response." },
      { status: 500 }
    );
  }
}
