...
import { getVerticalStructuralElementsCostCodeHierarchy } from "@/features/dashboard/services/get-vertical-structural-elements-cost-code-hierarchy";
import type {
...
type VerticalStructuralElementsReviewRow = {
  item: string;
  quantity: string;
  unit: "cu.m" | "ton";
...
type JobEstimateVerticalStructuralElementsEstimateBranchProps = {
...
const defaultReviewRow: VerticalStructuralElementsReviewRow = {
  item: "Vertical Structural Elements",
  quantity: "",
  unit: "cu.m",
...
export function JobEstimateVerticalStructuralElementsEstimateBranch({
  estimate,
  itemOnly = false,
  grossFloorArea,
...
}: JobEstimateVerticalStructuralElementsEstimateBranchProps) {
  ...
  const [reviewRow, setReviewRow] =
    useState<VerticalStructuralElementsReviewRow>(defaultReviewRow);
    ...
    async function loadBranch() {
      ...
      try {
        const [
          ...
          getVerticalStructuralElementsCostCodeHierarchy(),
          getJobEstimateDetailedItem(estimate.id, "B1017"),
        ]);
        ...
        const initialReviewRow = buildReviewRow(
          savedEstimate,
          loadedProjectDetails.superstructureType
        );
        setReviewRow(initialReviewRow);
        setPersistedSignature(createSignature(initialReviewRow));
      } catch (error) {
        console.error(
          "Failed to load vertical structural elements estimate branch:",
          error
        );
  ...
  const equipmentHeader = `Equipment / ${reviewRow.unit}`;
  const totalHeader = `Total / ${reviewRow.unit}`;
    ...
    try {
      await saveJobEstimateDetailedItem({
        ...
        rows: [
          {
            rowKey: "b1017-main",
            rowLabel: "Vertical Structural Elements",
  ...
  async function handleGenerateDraft() {
    ...
    setGenerationError("");
    setGenerationStatusMessage("Generating vertical structural elements draft...");

    try {
      const response = await fetch(
        "/api/job-estimates/vertical-structural-elements-draft",
        {
          method: "POST",
          headers: {
      ...
      >(response, "Failed to generate vertical structural elements draft.");

      setReviewRow((previousRow) => ({
        ...previousRow,
        item: payload?.item?.trim() || "Vertical Structural Elements",
        quantity: formatQuantityValue(payload?.quantity ?? 0),
        unit: payload?.unit ?? getDefaultUnit(projectDetails.superstructureType),
      ...
      setGenerationError(
        error instanceof Error
          ? error.message
          : "Failed to generate vertical structural elements draft."
      );
      setGenerationStatusMessage("");
    } finally {
  ...
  function handleRowChange(
    key: keyof Pick<
      VerticalStructuralElementsReviewRow,
      | "quantity"
      | "materialCostPerUnit"
      | "labourCostPerUnit"
      | "equipmentCostPerUnit"
  ...
  if (isLoading) {
    return (
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--panel-soft)] p-4 text-sm text-[var(--muted)]">
        Loading Vertical Structural Elements estimate branch...
      </section>
    );
  }
          ...
          >
            <JobEstimateHierarchyNode
              ...
              badge={buildEstimateBadges({
                ...
                totalQuantity,
                grossFloorArea,
                quantityUnitLabel: reviewRow.unit,
              })}
              isOpen={isItemOpen}
                ...
                <div className="flex flex-col gap-3 border-b border-[var(--border)] pb-4 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <h4 className="text-lg font-semibold">
                      Vertical Structural Elements Cost Breakdown Review
                    </h4>
                    <p className="text-sm text-[var(--muted)]">
                      Generate a draft using project details, area takeoffs, and
                      finishes. The quantity and unit switch automatically based on
                      the selected superstructure type.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    >
                      {isGeneratingDraft
                        ? "Generating Draft..."
                        : "Generate Vertical Elements Draft"}
                    </button>
                    <button
                      type="button"
                    ...
                    <tfoot>
                      <tr className="bg-[var(--surface)]">
                        <td className="px-3 py-3 font-semibold" colSpan={7}>
                          Vertical Structural Elements Branch Total
                        </td>
...
function getDefaultUnit(superstructureType: string) {
  return superstructureType.trim().toLowerCase() === "steel" ? "ton" : "cu.m";
}
...
function buildStatusLabel(row: {
  confidence: VerticalStructuralElementsReviewRow["confidence"];
  status: string;
}) {
...
function buildReviewRow(
  ...
  if (!savedRow) {
    return {
      ...defaultReviewRow,
      unit: getDefaultUnit(superstructureType),
    };
  }

  return {
    item: savedEstimate?.item.itemName ?? "Vertical Structural Elements",
    quantity: formatQuantityValue(savedRow.quantity),
    unit: savedRow.unit === "ton" ? "ton" : "cu.m",
...
function createSignature(row: VerticalStructuralElementsReviewRow) {
