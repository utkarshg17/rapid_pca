...
type ElectricalCondutingReviewRow = {
  item: string;
  quantity: string;
  unit: "LF";
...
type JobEstimateElectricalCondutingEstimateBranchProps = {
...
const defaultReviewRow: ElectricalCondutingReviewRow = {
  item: "Electrical Conduting",
  quantity: "",
  unit: "LF",
...
export function JobEstimateElectricalCondutingEstimateBranch({
  estimate,
  itemOnly = false,
  grossFloorArea,
  ...
  const [reviewRow, setReviewRow] =
    useState<ElectricalCondutingReviewRow>(defaultReviewRow);
    ...
    async function loadBranch() {
      ...
      try {
        const [
          ...
          getElectricalCondutingCostCodeHierarchy(),
          getJobEstimateDetailedItem(estimate.id, "D5013"),
        ]);
        ...
        console.error("Failed to load electrical conduting estimate branch:", error);
  ...
  async function handleGenerateDraft() {
    ...
    setGenerationError("");
    setGenerationStatusMessage("Generating electrical conduting draft...");

    try {
      const response = await fetch("/api/job-estimates/electrical-conduting-draft", {
        method: "POST",
        headers: {
      ...
      setReviewRow((previousRow) => ({
        ...previousRow,
        item: payload?.item?.trim() || "Electrical Conduting",
      ...
      setGenerationError(
        error instanceof Error
          ? error.message
          : "Failed to generate electrical conduting draft."
      );
      setGenerationStatusMessage("");
    } finally {
  ...
  if (isLoading) {
    return (
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--panel-soft)] p-4 text-sm text-[var(--muted)]">
        Loading Electrical Conduting estimate branch...
      </section>
    );
  }
                ...
                <div className="flex flex-col gap-3 border-b border-[var(--border)] pb-4 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <h4 className="text-lg font-semibold">
                      Electrical Conduting Cost Breakdown Review
                    </h4>
                    <p className="text-sm text-[var(--muted)]">
                      Generate a draft using project details, area takeoffs, and
                      finishes. This scope includes conduiting pipe, wiring, and
                      distribution boxes only, not switches or fixtures.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    >
                      {isGeneratingDraft
                        ? "Generating Draft..."
                        : "Generate Electrical Conduting Draft"}
                    </button>
                    <button
                      type="button"
...
function buildStatusLabel(row: {
  confidence: ElectricalCondutingReviewRow["confidence"];
  status: string;
}) {
...
function buildReviewRow(
  ...
  return {
    item: savedEstimate?.item.itemName ?? "Electrical Conduting",
...
function createSignature(row: ElectricalCondutingReviewRow) {
