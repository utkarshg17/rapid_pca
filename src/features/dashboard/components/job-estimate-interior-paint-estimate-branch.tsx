...
import { getInteriorPaintCostCodeHierarchy } from "@/features/dashboard/services/get-interior-paint-cost-code-hierarchy";
...
type JobEstimateInteriorPaintEstimateBranchProps = {
...
const defaultReviewRow: InteriorPaintReviewRow = {
  item: "Interior Paint",
  area: "",
  finishDescription: "",
...
export function JobEstimateInteriorPaintEstimateBranch({
  estimate,
  itemOnly = false,
  grossFloorArea,
...
registerBulkSave,
  ...
  const [reviewRow, setReviewRow] =
    useState<InteriorPaintReviewRow>(defaultReviewRow);
    ...
    async function loadBranch() {
      ...
      try {
        const [
          ...
          getInteriorPaintCostCodeHierarchy(),
          getJobEstimateDetailedItem(estimate.id, "C3015"),
        ]);
        ...
        const initialReviewRow = buildReviewRow(savedEstimate, finishDescriptionFromRows(loadedFinishes, "interior paint"));
        setReviewRow(initialReviewRow);
        setPersistedSignature(createSignature(initialReviewRow));
      } catch (error) {
        console.error("Failed to load interior paint estimate branch:", error);
  ...
  const finishDescription = useMemo(
    () =>
      finishes
        .filter(
          (row) =>
            row.finishType.trim().toLowerCase() === "interior paint" &&
  ...
  async function handleGenerateDraft() {
    ...
    setGenerationError("");
    setGenerationStatusMessage("Generating interior paint draft...");
    ...
    try {
      ...
      setReviewRow((previousRow) => ({
        ...previousRow,
        item: payload?.item?.trim() || "Interior Paint",
      ...
      setGenerationError(
        error instanceof Error
          ? error.message
          : "Failed to generate interior paint draft."
      );
      setGenerationStatusMessage("");
    } finally {
  ...
  if (isLoading) {
    return (
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--panel-soft)] p-4 text-sm text-[var(--muted)]">
        Loading Interior Paint estimate branch...
      </section>
    );
  }
                ...
                <div className="flex flex-col gap-3 border-b border-[var(--border)] pb-4 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <h4 className="text-lg font-semibold">
                      Interior Paint Cost Breakdown Review
                    </h4>
                    <p className="text-sm text-[var(--muted)]">
                      Generate a draft based on project details, area takeoffs, and
                      the interior paint finish description.
                    </p>
                  </div>
...
function buildStatusLabel(row: {
  confidence: InteriorPaintReviewRow["confidence"];
  status: string;
}) {
...
function buildStatusClassName(confidence: InteriorPaintReviewRow["confidence"]) {
  const baseClassName =
...
function buildReviewRow(
  ...
  return {
    item: savedEstimate?.item.itemName ?? "Interior Paint",
...
function createSignature(row: InteriorPaintReviewRow) {
