...
import { getColumnFoundationsFootingsCostCodeHierarchy } from "@/features/dashboard/services/get-column-foundations-footings-cost-code-hierarchy";
...
type ColumnFoundationsFootingsReviewRow = {
  item: string;
  quantity: string;
...
type JobEstimateColumnFoundationsFootingsEstimateBranchProps = {
...
const defaultReviewRow: ColumnFoundationsFootingsReviewRow = {
  item: "Column Foundations + Footings",
  quantity: "",
...
export function JobEstimateColumnFoundationsFootingsEstimateBranch({
  estimate,
  itemOnly = false,
  grossFloorArea,
  ...
  const [reviewRow, setReviewRow] =
    useState<ColumnFoundationsFootingsReviewRow>(defaultReviewRow);
    ...
    async function loadBranch() {
      ...
      try {
        const [
          ...
          getColumnFoundationsFootingsCostCodeHierarchy(),
          getJobEstimateDetailedItem(estimate.id, "A1012"),
        ]);
        ...
        console.error(
          "Failed to load column foundations + footings estimate branch:",
          error
        );
      ...
      await saveJobEstimateDetailedItem({
        ...
        rows: [
          ...
          unit: "cu.m",
            ...
            totalCostPerUnit: calculateRatePerCum(reviewRow),
  ...
  async function handleGenerateDraft() {
    ...
    setGenerationStatusMessage(
      "Generating column foundations + footings draft..."
    );

    try {
      const response = await fetch(
        "/api/job-estimates/column-foundations-footings-draft",
        {
          method: "POST",
          headers: {
      ...
      setReviewRow((previousRow) => ({
        ...previousRow,
        item: payload?.item?.trim() || "Column Foundations + Footings",
        quantity: formatQuantityValue(payload?.quantityCum ?? 0),
        assumedSystem: payload?.assumedSystem?.trim() || "Pending AI",
        materialCostPerCum: formatCurrencyNumber(payload?.materialCostPerCum ?? 0),
        labourCostPerCum: formatCurrencyNumber(payload?.labourCostPerCum ?? 0),
        equipmentCostPerCum: formatCurrencyNumber(
          payload?.equipmentCostPerCum ?? 0
        ),
        assumptions: payload?.assumptions?.trim() || "Awaiting AI draft",
        confidence: payload?.confidence ?? "pending",
        status:
          (payload?.quantityCum ?? 0) === 0 &&
          !isApplicableFoundationType(projectDetails.foundationType)
            ? "Not applicable"
            : "AI drafted",
      }));
  ...
  function handleRowChange(
    key: keyof Pick<
      ColumnFoundationsFootingsReviewRow,
  ...
  if (isLoading) {
    return (
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--panel-soft)] p-4 text-sm text-[var(--muted)]">
        Loading Column Foundations + Footings estimate branch...
      </section>
    );
  }
                ...
                <div className="flex flex-col gap-3 border-b border-[var(--border)] pb-4 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <h4 className="text-lg font-semibold">
                      Column Foundations + Footings Cost Breakdown Review
                    </h4>
                    <p className="text-sm text-[var(--muted)]">
                      Generate a draft using project details, area takeoffs, and
                      finishes. This item will only price when the project uses
                  ...
                  <table className="min-w-[1040px] divide-y divide-[var(--border)] text-left text-sm">
                    ...
                    <tbody className="divide-y divide-[var(--border)]">
                      <tr>
                        ...
                        <td className="px-3 py-3 align-top whitespace-nowrap">
                          <Input
                            value={reviewRow.materialCostPerCum}
                            onChange={(event) =>
                              handleRowChange(
                                "materialCostPerCum",
                                event.target.value
                        ...
                        <td className="px-3 py-3 align-top whitespace-nowrap">
                          <Input
                            value={reviewRow.labourCostPerCum}
                            onChange={(event) =>
                              handleRowChange("labourCostPerCum", event.target.value)
                            }
                            inputMode="decimal"
                            placeholder="0"
                            className="h-10 min-w-[8rem] rounded-xl px-3 py-2 text-xs"
                        ...
                        <td className="px-3 py-3 align-top whitespace-nowrap">
                          <Input
                            value={reviewRow.equipmentCostPerCum}
                            onChange={(event) =>
                              handleRowChange(
                                "equipmentCostPerCum",
                                event.target.value
                        ...
                        <td className="px-3 py-3 align-top whitespace-nowrap text-[var(--muted)]">
                          INR {formatCurrencyNumber(calculateRatePerCum(reviewRow))}
                        </td>
                        <td className="px-3 py-3 align-top whitespace-nowrap text-[var(--muted)]">
...
function isApplicableFoundationType(value: string) {
  const normalizedValue = value.trim().toLowerCase();
  return (
    normalizedValue === "isolated footing" ||
    normalizedValue === "isolated footing + combined footing"
  );
}
...
function buildStatusLabel(row: {
  confidence: ColumnFoundationsFootingsReviewRow["confidence"];
  status: string;
}) {
...
function buildReviewRow(
  ...
  return {
    item: savedEstimate?.item.itemName ?? "Column Foundations + Footings",
