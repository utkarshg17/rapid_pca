...
import { getRegularStairCostCodeHierarchy } from "@/features/dashboard/services/get-regular-stair-cost-code-hierarchy";
...
type JobEstimateRegularStairEstimateBranchProps = {
...
const defaultReviewRow: RegularStairReviewRow = {
  item: "Regular Stair",
  quantity: "",
...
export function JobEstimateRegularStairEstimateBranch({
  estimate,
  itemOnly = false,
  grossFloorArea,
  ...
  const [reviewRow, setReviewRow] = useState<RegularStairReviewRow>(defaultReviewRow);
    ...
    async function loadBranch() {
      ...
      try {
        const [
          ...
          getRegularStairCostCodeHierarchy(),
          getJobEstimateDetailedItem(estimate.id, "C2011"),
        ]);
        ...
        console.error("Failed to load regular stair estimate branch:", error);
  ...
  async function handleGenerateDraft() {
    ...
    setGenerationError("");
    setGenerationStatusMessage("Generating regular stair draft...");

    try {
      const response = await fetch("/api/job-estimates/regular-stair-draft", {
        method: "POST",
        headers: {
      ...
      setReviewRow((previousRow) => ({
        ...previousRow,
        item: payload?.item?.trim() || "Regular Stair",
        ...
        equipmentCostPerCum: formatCurrencyNumber(payload?.equipmentCostPerCum ?? 0),
      ...
      setGenerationError(
        error instanceof Error
          ? error.message
          : "Failed to generate regular stair draft."
      );
  ...
  if (isLoading) {
    return (
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--panel-soft)] p-4 text-sm text-[var(--muted)]">
        Loading Regular Stair estimate branch...
      </section>
    );
  }
                ...
                <div className="flex flex-col gap-3 border-b border-[var(--border)] pb-4 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <h4 className="text-lg font-semibold">
                      Regular Stair Cost Breakdown Review
                    </h4>
                    <p className="text-sm text-[var(--muted)]">
                      Generate a draft using project details, area takeoffs, and
                      finishes. This item covers the structural RCC stair only and
                      excludes any finish layers applied on top.
                    </p>
                  </div>
                      ...
                      <tr>
                        ...
                        <td className="px-3 py-3 align-top whitespace-nowrap">
                          <Input
                            ...
                            onChange={(event) =>
                              handleRowChange("materialCostPerCum", event.target.value)
                            }
                            inputMode="decimal"
                            placeholder="0"
                            className="h-10 min-w-[8rem] rounded-xl px-3 py-2 text-xs"
                        ...
                        <td className="px-3 py-3 align-top whitespace-nowrap">
                          <Input
                            ...
                            onChange={(event) =>
                              handleRowChange("equipmentCostPerCum", event.target.value)
                            }
                            inputMode="decimal"
                            placeholder="0"
                            className="h-10 min-w-[8rem] rounded-xl px-3 py-2 text-xs"
...
function buildStatusLabel(row: {
  confidence: RegularStairReviewRow["confidence"];
  status: string;
}) {
...
function buildReviewRow(
  ...
  return {
    item: savedEstimate?.item.itemName ?? "Regular Stair",
