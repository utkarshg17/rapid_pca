...
import { getInteriorPlasterCostCodeHierarchy } from "@/features/dashboard/services/get-interior-plaster-cost-code-hierarchy";
...
type JobEstimateInteriorPlasterEstimateBranchProps = {
...
const defaultReviewRow: InteriorPlasterReviewRow = {
  item: "Interior Plaster",
  area: "",
  finishDescription: "",
...
export function JobEstimateInteriorPlasterEstimateBranch({
  estimate,
  itemOnly = false,
  grossFloorArea,
  ...
  savedById,
  savedByName,
}: JobEstimateInteriorPlasterEstimateBranchProps) {
  ...
  const [reviewRow, setReviewRow] =
    useState<InteriorPlasterReviewRow>(defaultReviewRow);
    ...
    async function loadBranch() {
      ...
      try {
        const [
          ...
          getInteriorPlasterCostCodeHierarchy(),
          getJobEstimateDetailedItem(estimate.id, "C3014"),
        ]);
        ...
        const initialReviewRow = buildReviewRow(savedEstimate, finishDescriptionFromRows(loadedFinishes, "interior plaster"));
        setReviewRow(initialReviewRow);
        setPersistedSignature(createSignature(initialReviewRow));
      } catch (error) {
        console.error("Failed to load interior plaster estimate branch:", error);
  ...
  const finishDescription = useMemo(
    () =>
      finishes
        .filter(
          (row) =>
            row.finishType.trim().toLowerCase() === "interior plaster" &&
  ...
  async function handleGenerateDraft() {
    ...
    setGenerationError("");
    setGenerationStatusMessage("Generating interior plaster draft...");

    try {
      const response = await fetch("/api/job-estimates/interior-plaster-draft", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(
          payload?.error || "Failed to generate interior plaster draft."
        );
      }

      setReviewRow((previousRow) => ({
        ...previousRow,
        item: payload?.item?.trim() || "Interior Plaster",
      ...
      setGenerationError(
        error instanceof Error
          ? error.message
          : "Failed to generate interior plaster draft."
      );
      setGenerationStatusMessage("");
    } finally {
  ...
  if (isLoading) {
    return (
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--panel-soft)] p-4 text-sm text-[var(--muted)]">
        Loading Interior Plaster estimate branch...
      </section>
    );
  }
                ...
                <div className="flex flex-col gap-3 border-b border-[var(--border)] pb-4 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <h4 className="text-lg font-semibold">
                      Interior Plaster Cost Breakdown Review
                    </h4>
                    <p className="text-sm text-[var(--muted)]">
                      Generate a draft based on project details, area takeoffs, and
                      the interior plaster finish description.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    >
                      {isGeneratingDraft
                        ? "Generating Draft..."
                        : "Generate Interior Plaster Draft"}
                    </button>
                    <button
                      type="button"
...
function buildStatusLabel(row: {
  confidence: InteriorPlasterReviewRow["confidence"];
  status: string;
}) {
...
function buildStatusClassName(confidence: InteriorPlasterReviewRow["confidence"]) {
  const baseClassName =
...
function buildReviewRow(
  ...
  return {
    item: savedEstimate?.item.itemName ?? "Interior Plaster",
...
function createSignature(row: InteriorPlasterReviewRow) {
