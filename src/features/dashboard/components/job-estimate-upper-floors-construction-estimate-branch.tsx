...
import { getUpperFloorsConstructionCostCodeHierarchy } from "@/features/dashboard/services/get-upper-floors-construction-cost-code-hierarchy";
...
type UpperFloorsConstructionReviewRow = {
  item: string;
  quantity: string;
  unit: "cu.m" | "ton";
...
type JobEstimateUpperFloorsConstructionEstimateBranchProps = {
...
const defaultReviewRow: UpperFloorsConstructionReviewRow = {
  item: "Upper Floors Construction (Slab + Beam)",
  quantity: "",
  unit: "cu.m",
...
export function JobEstimateUpperFloorsConstructionEstimateBranch({
  estimate,
  itemOnly = false,
  grossFloorArea,
  ...
  const [reviewRow, setReviewRow] =
    useState<UpperFloorsConstructionReviewRow>(defaultReviewRow);
    ...
    async function loadBranch() {
      ...
      try {
        const [
          ...
          getUpperFloorsConstructionCostCodeHierarchy(),
          getJobEstimateDetailedItem(estimate.id, "B1012"),
        ]);
        ...
        console.error(
          "Failed to load upper floors construction estimate branch:",
          error
        );
  ...
  async function handleGenerateDraft() {
    ...
    setGenerationError("");
    setGenerationStatusMessage("Generating upper floors construction draft...");

    try {
      const response = await fetch(
        "/api/job-estimates/upper-floors-construction-draft",
        {
          method: "POST",
          headers: {
      ...
      setReviewRow((previousRow) => ({
        ...previousRow,
        item: payload?.item?.trim() || "Upper Floors Construction (Slab + Beam)",
  ...
  function handleRowChange(
    key: keyof Pick<
      UpperFloorsConstructionReviewRow,
      | "quantity"
      | "materialCostPerUnit"
      | "labourCostPerUnit"
      | "equipmentCostPerUnit"
  ...
  if (isLoading) {
    return (
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--panel-soft)] p-4 text-sm text-[var(--muted)]">
        Loading Upper Floors Construction estimate branch...
      </section>
    );
  }
                ...
                <div className="flex flex-col gap-3 border-b border-[var(--border)] pb-4 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <h4 className="text-lg font-semibold">
                      Upper Floors Construction (Slab + Beam) Cost Breakdown Review
                    </h4>
...
function buildStatusLabel(row: {
  confidence: UpperFloorsConstructionReviewRow["confidence"];
  status: string;
}) {
...
function buildReviewRow(
  ...
  return {
    item: savedEstimate?.item.itemName ?? "Upper Floors Construction (Slab + Beam)",
...
function createSignature(row: UpperFloorsConstructionReviewRow) {
