...
import { getInteriorWindowsCostCodeHierarchy } from "@/features/dashboard/services/get-interior-windows-cost-code-hierarchy";
...
type JobEstimateInteriorWindowsEstimateBranchProps = {
...
export function JobEstimateInteriorWindowsEstimateBranch({
  estimate,
  itemOnly = false,
  grossFloorArea,
...
}: JobEstimateInteriorWindowsEstimateBranchProps) {
  ...
  const [reviewRows, setReviewRows] = useState<InteriorWindowReviewRow[]>([]);
    ...
    async function loadBranch() {
      ...
      try {
        const [
          ...
          getInteriorWindowsCostCodeHierarchy(),
          getJobEstimateProjectDetails(estimate),
          getJobEstimateDetailedItem(estimate.id, "C1017"),
        ]);
        ...
        console.error("Failed to load interior windows estimate branch:", error);
    ...
    try {
      await saveJobEstimateDetailedItem({
        ...
        rows: reviewRows.map((row, index) => ({
          rowKey: `interior-windows-${row.id}`,
          ...
          assumedSystem: row.assumedWindowSystem,
  ...
  async function handleGenerateDraft() {
    ...
    setGenerationError("");
    setGenerationStatusMessage("Generating interior windows draft...");

    try {
      const response = await fetch("/api/job-estimates/interior-windows-draft", {
        method: "POST",
        headers: {
      ...
      >(response, "Failed to generate interior windows draft.");
      ...
      setReviewRows((previousRows) =>
        previousRows.map((row) => {
          ...
          return {
            ...row,
            assumedWindowSystem: generatedRow.assumedWindowSystem,
      ...
      setGenerationError(
        error instanceof Error
          ? error.message
          : "Failed to generate interior windows draft."
      );
  ...
  if (isLoading) {
    return (
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--panel-soft)] p-4 text-sm text-[var(--muted)]">
        Loading Interior Windows estimate branch...
      </section>
    );
  }
              ...
              <section className="rounded-2xl border border-[var(--border)] bg-[var(--panel)] p-4">
                <div className="flex flex-col gap-3 border-b border-[var(--border)] pb-4 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <h4 className="text-lg font-semibold">Interior Windows Cost Breakdown Review</h4>
                    <p className="text-sm text-[var(--muted)]">
                      Generate a draft using project details, area takeoffs, finishes,
                      and the saved window openings. Quantity comes directly from the opening area.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    >
                      {isGeneratingDraft ? "Generating Draft..." : "Generate Interior Windows Draft"}
                    </button>
                    <button
                      type="button"
                ...
                <div className="mt-4 overflow-x-auto">
                  <table className="min-w-[1180px] divide-y divide-[var(--border)] text-left text-sm">
                    ...
                    <tbody className="divide-y divide-[var(--border)]">
                      {reviewRows.length > 0 ? (
                        reviewRows.map((row) => (
                          <tr key={`interior-windows-${row.id}`}>
                            <td className="px-3 py-3 whitespace-normal break-words">
                              <p className="font-medium">{row.openingName}</p>
                              <p className="mt-1 text-xs text-[var(--muted)]">{row.assumedWindowSystem}</p>
                        ...
                        <tr>
                          <td colSpan={9} className="px-3 py-6 text-center text-sm text-[var(--muted)]">
                            Add window rows in the Openings tab to generate the Interior Windows estimate.
                          </td>
                        </tr>
                      )}
                    </tbody>
...
function buildStatusLabel(row: {
  confidence: InteriorWindowReviewRow["confidence"];
  status: string;
}) {
...
function buildStatusClassName(confidence: InteriorWindowReviewRow["confidence"]) {
  const baseClassName =
...
function buildReviewRows(
  ...
  existingRows: InteriorWindowReviewRow[] = []
) {
  ...
  return openings
    ...
    .map((row) => {
      const rowKey = `interior-windows-${row.id}`;
      ...
      return {
        id: row.id,
        openingName: row.openingName || savedRow?.rowLabel || "Untitled window",
        areaSqft: formatArea(savedRow?.quantity ?? areaSqft),
        description: row.description || "No description provided",
        assumedWindowSystem:
          savedRow?.assumedSystem || existingRow?.assumedWindowSystem || "Pending AI",
        materialCostPerSqft:
          savedRow != null
...
function createSignature(rows: InteriorWindowReviewRow[]) {
  return JSON.stringify(
    rows.map((row) => ({
      ...
      description: row.description,
      assumedWindowSystem: row.assumedWindowSystem,
